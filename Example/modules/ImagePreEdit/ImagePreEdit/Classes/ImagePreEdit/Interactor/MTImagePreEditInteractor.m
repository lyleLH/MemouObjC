...
- (void)preparePreMergeData {
    [self.stitcher mergeImages:self.dataManager.originalImages completion:^(SZImageGenerator * _Nonnull generator, NSError * _Nonnull error) {
        if(generator){
            [self.presenter preEditImagePrepared:generator];
        }
    }];
}
- (void)mergeBegin { 
